...
public class KalmanFilterFloat : MonoBehaviour{
	...
	public float Filter(float measurement, float? newQ = null, float? newR = null) {
	...
	public float Filter(List<float> measurements, bool areMeasurementsNewestFirst = false, float? newQ = null, float? newR = null) {
		...
		while (i < measurements.Count && i >= 0) {
			...
			result = Filter(measurements[i], newQ, newR);
		}
