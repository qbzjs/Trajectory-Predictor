...
namespace Autohand {
    ...
    public class Hand : HandBase {
        ...
        protected override void Update() {
            ...
            //     if(holdingObj && !holdingObj.maintainGrabOffset && !IsGrabbing()) {
            //         var deltaDist = Vector3.Distance(follow.position, lastFrameFollowPos);
            //         var deltaRot = Quaternion.Angle(gameObject.GetComponent<FollowRotation>().targetRotation, lastFrameFollowRot);
            //         grabPositionOffset = Vector3.MoveTowards(grabPositionOffset, Vector3.zero, (deltaDist) * gentleGrabSpeed * Time.deltaTime * 60f);
            //         grabRotationOffset = Quaternion.RotateTowards(grabRotationOffset, gameObject.GetComponent<FollowRotation>().targetRotation, (deltaRot) * gentleGrabSpeed * Time.deltaTime * 60f);
            //
            //         if(!holdingObj.useGentleGrab) {
            //             grabPositionOffset = Vector3.MoveTowards(grabPositionOffset, Vector3.zero, Time.deltaTime / GetGrabTime());
            //             grabRotationOffset = Quaternion.RotateTowards(grabRotationOffset, gameObject.GetComponent<FollowRotation>().targetRotation, (90f * Time.deltaTime) / GetGrabTime());
            //         }
            //     }
            //
            //     lastFrameFollowPos = follow.position;
            //     lastFrameFollowRot = gameObject.GetComponent<FollowRotation>().targetRotation;
            // }
            // base.Update();
        }
